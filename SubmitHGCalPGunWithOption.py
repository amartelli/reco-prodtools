...
def parseOptions():
    ...
    parser.add_option(  '', '--outArea',dest='outArea',type='string', default='/eos/cms/store/cmst3/group/hgcal/',help='path to the eos area where the output jobs will be stagedout, different from eosArea')
    ...
    parser.add_option('-o', '--outDir',  dest='outDir',  type='string', default='',  help='name of the out stage dir (relative to the local submission or "eosArea"), to be used as the input for next stage, not applicable for GEN stage')
...
def submitHGCalProduction():
    ...
    if (opt.LOCAL):
        ...
        processCmd(eosExec + ' mkdir -p '+opt.outArea+'/'+outDir+'/'+opt.DTIER+'/');
    ...
    for particle in particles:
        ...
        for job in range(1,int(njobs)+1):
            ...
            processCmd("sed -i 's~DUMMYSEED~"+str(job+100)+"~g' "+outDir+'/cfg/'+cfgfile)
            if (opt.DTIER == 'GSD'):
            ...
            # submit job
            cmd = 'bsub -o '+outDir+'/std/'+basename +'.out -e '+outDir+'/std/'+basename +'.err -q '+opt.QUEUE+' -J '+basename+' "SubmitFileGSDWithOption.sh '+currentDir+' '+outDir+' '+cfgfile+' '+str(opt.LOCAL)+' '+CMSSW_VERSION+' '+CMSSW_BASE+' '+SCRAM_ARCH+' '+opt.eosArea+' '+opt.outArea+' '+opt.DTIER+'"'
